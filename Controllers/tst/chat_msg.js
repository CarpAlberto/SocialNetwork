 function Add_New_Window(label,name,id,image,conf_id)
 {
   ...
   if(a == 1) 
    ...
    url = 'message/GetHistory/' + id;
    if(conf_id != null)
    {
           url = 'conferinta/GetHistory/' + conf_id;
    }
      ...
      $.ajax({
                      ...
                      success: function(response)
                      {
                         ...
                         for (var i = 0;i<response.length;i++) {
                             ...
                             if(response[i].Sender == _ID)
                             {
                              
                                  WriteSendMessage($('#Wrapper_Msg').find('.shout_box').last(),response[i].Mesaj,response[i].Media)
                             }
                             else
                             {
                                
                                  WriteGetMessage($('#Wrapper_Msg').find('.shout_box').last(),response[i].Mesaj,response[i].Photo_Profil,response[i].Media)
                             }
                         };
                      }
                    });
   ...
   if(localStorage['level'] == 4)
  {
   ...
   $.ajax({
               ...
               success: function(php_script_response)
                {
                  alert('Fisierul' + php_script_response + 'a fost uploadat cu succes.Adaugati text si tastati Enter pentru a trimite mesajul');
...
function UpMessage(evt,name,id)
{
   if(evt.which == 13) 
   {
                ...
                var url = 'message/Send/' + imessage + "/" + _ID + '/' + id + '/' + media;
                if(localStorage['level'] == 3)
                {
                    url = '../message/Send/' + imessage + "/" + _ID + '/' + id + '/' + media
                }
                if(localStorage['level'] == 4)
                {
                    url = '../../message/Send/' + imessage + "/" + _ID + '/' + id + '/' + media
                        ...
                        if(data.length > 0)
                         {
                            for (var i =0 ;i< data.length;i++) 
                            {
                               ...
                               {
                                 if($(this).attr('title').trim() == data[i].Sender.trim())
                                 {
